...
`docker logs -f` 默认只显示 `notice` 及以上级别，避免刷太多细节。

容器里的日志也会写到本地挂载目录：

- `runtime/klei/logs/container.log`：和 `docker logs` 同源的入口脚本日志
- `runtime/klei/logs/master.log`：Master 分片完整日志
- `runtime/klei/logs/caves.log`：Caves 分片完整日志
- `runtime/klei/logs/steamcmd.log`：SteamCMD 更新日志

如果要让 `docker logs -f` 显示更多内容，可以启动容器时加：

```bash
-e DST_CONSOLE_LOG_LEVEL=info
```

可选级别：`error`、`warn`、`notice`、`info`、`debug`、`off`。默认 `DST_CONSOLE_LOG_LEVEL=notice`，本地 `container.log` 默认写到 `debug`。
