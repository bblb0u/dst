...
container_log="${DST_CONTAINER_LOG_FILE:-$log_dir/container.log}"
console_log_level="${DST_CONSOLE_LOG_LEVEL:-notice}"
file_log_level="${DST_FILE_LOG_LEVEL:-debug}"
...
function should_log()
{
	...
	if ! level_priority="$(log_level_priority "$level")"; then
		level_priority=2
	fi

	if ! threshold_priority="$(log_level_priority "$threshold")"; then
		threshold_priority=2
	fi

	[ "$threshold_priority" -ge 0 ] && [ "$level_priority" -le "$threshold_priority" ]
}

function is_log_level()
{
	log_level_priority "$1" >/dev/null
}

function write_container_log()
{
	local line="$1"

	[ -n "$container_log" ] || return 0

	mkdir -p "$(dirname "$container_log")" 2>/dev/null || true
	printf '%s\n' "$line" >> "$container_log" 2>/dev/null || true
}
...
function log_message()
{
	...
	line="$(printf '%s [dst] %-6s %s' "$timestamp" "${level^^}" "$*")"

	if should_log "$level" "$file_log_level"; then
		write_container_log "$line"
	fi

	if should_log "$level" "$console_log_level"; then
		if [ "${level,,}" = "error" ]; then
			printf '%s\n' "$line" >&2
		else
			printf '%s\n' "$line"
...
function validate_runtime_config()
{
	...
	is_log_level "$console_log_level" \
		|| fail "DST_CONSOLE_LOG_LEVEL must be one of: error, warn, notice, info, debug, off."
	is_log_level "$file_log_level" \
		|| fail "DST_FILE_LOG_LEVEL must be one of: error, warn, notice, info, debug, off."
...
function perform_auto_backup()
{
	...
	notice "Auto backup triggered for Day $day. Archiving current cluster state."
...
EOF
	...
	notice "Auto backup stored Day $day: $archive_path"
}
...
function initialize_template_identifiers()
{
	...
	if [ -f "$cluster_ini" ]; then
		...
		debug "Generated unique cluster_cloud_id for this deployment."
	fi

	if [ -f "$caves_server_ini" ] && grep -Eq '^[[:space:]]*id[[:space:]]*=' "$caves_server_ini"; then
		sed -i -E '/^[[:space:]]*id[[:space:]]*=/d' "$caves_server_ini"
		debug "Removed hard-coded Caves shard id so DST can auto-generate a unique value."
	fi
...
function seed_default_cluster_if_needed()
{
	if [ -d "$cluster_dir" ]; then
		debug "Existing cluster found. Keeping current data in $cluster_dir."
		return
	fi

	check_for_directory "$default_cluster_dir"

	notice "No cluster found. Seeding default template for $cluster_name."
...
function start_shards()
{
	...
	notice "Detailed logs: $log_dir"
	notice "Container log: $container_log"
	notice "Starting shards: Caves, Master"
...
function wait_for_shards()
{
	...
	notice "$exited_shard shard exited with status $status. Check $log_dir for details."
	exit "$status"
}
...
function handle_shutdown()
{
	...
	notice "Shutdown signal received. Asking shards to save and stop."
	request_graceful_shutdown

	if wait_for_shards_to_exit "$shutdown_timeout"; then
		notice "Shards stopped cleanly."
	else
		warn "Graceful shard shutdown timed out after ${shutdown_timeout}s. Forcing stop."
	fi
